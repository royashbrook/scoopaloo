...
test('Rush HUD and order panel use the full safe width without clipping at every phone gate', async ({ page }) => {
  ...
  for (const size of PHONE_SIZES) {
    ...
    expect(layout.rail.width, size.name).toBeCloseTo(60, 0)
    expect(layout.gap, size.name).toBeCloseTo(8, 0)
    expect(layout.ticket.width, size.name).toBeGreaterThanOrEqual(layout.width - 100)
