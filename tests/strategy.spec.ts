...
test('plans two orders ahead and makes combo money an audible mobile stake', async ({ page }) => {
  ...
  expect(phone.ticket.left - phone.rail.right).toBeCloseTo(8, 0)
  expect(phone.railBeforeTicket).toBe(true)
  expect(phone.ticket.height).toBeLessThanOrEqual(244) // current recipe ticket is 242.625px; preview must not extend it
  ...
  expect(phone.iconWidth).toBeGreaterThanOrEqual(30)
