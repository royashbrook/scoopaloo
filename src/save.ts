...
import { decodeSaveCode, encodeSaveCode, SAVE_KEY } from './save-code'
...
export async function encodeSave(save: SaveV1): Promise<string> {
  return encodeSaveCode({ ...save, text: true })
}

export async function decodeSave(skin: GameSkin, code: string): Promise<SaveV1> {
  return migrateSave(skin, await decodeSaveCode(code) as Partial<SaveV1>)
}
